...
const ONBOARDING_FLAG_KEY = 'needsOnboarding'; // Add a constant for the key
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
    ...
    useEffect(() => {
      const handleRedirect = async () => {
        ...
        if (authState.authenticated) {
          if (inAuthGroup) {
            const needsOnboarding = await SecureStore.getItemAsync(ONBOARDING_FLAG_KEY);
  ...
  const signup = async (username: string, email: string, password: string, likedMovieIds: number[] = []) => {
    ...
    try {
      ...
      // Set the flag indicating that the user has just signed up and needs onboarding.
      await SecureStore.setItemAsync(ONBOARDING_FLAG_KEY, 'true');
