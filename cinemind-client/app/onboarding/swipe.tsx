...
const SWIPE_COUNT = 10; // 스와이프 횟수를 10으로 정의
...
interface OnboardingMovie {
  ...
  poster_url: string;
  genre_name: string;
  actors: string[]; // 배우 필드 추가
}

export default function OnboardingSwipeScreen() {
  const router = useRouter();
  const { moodKeywords, moodName, moodEmoji } = useLocalSearchParams();
  ...
  useEffect(() => {
    const fetchOnboardingMovies = async () => {
        if (!moodKeywords) {
            setError("기분 키워드 정보가 없습니다.");
            setIsLoading(false);
            return;
        }
        try {
            const url = `${API_BASE_URL}/movies/onboarding?mood_keywords=${encodeURIComponent(moodKeywords as string)}`;
            const response = await fetch(url);
            if (!response.ok) {
                throw new Error('서버에서 영화 목록을 가져오는데 실패했습니다.');
            }
            const data: OnboardingMovie[] = await response.json();
            if (data.length < SWIPE_COUNT) { // 최소 스와이프 횟수보다 적으면 오류 처리
                throw new Error('추천할 영화가 충분하지 않습니다.');
            }
            setMovies(data);
        } catch (e: any) {
            setError(e.message);
  ...
  const advanceCard = (liked: boolean) => {
    ...
    // 5번 스와이프했거나, 준비된 영화를 모두 소진했으면 완료
    if (nextIndex >= SWIPE_COUNT || nextIndex >= movies.length) {
  ...
  // 진행도 계산 로직 수정
  const progressPercent = SWIPE_COUNT > 0 ? ((currentIndex) / SWIPE_COUNT) * 100 : 0;
  const displayTotal = Math.min(movies.length, SWIPE_COUNT);
      ...
      <SafeAreaView style={styles.safeArea}>
        <View style={styles.container}>
          <View style={styles.header}>
            <Text style={styles.title}>{moodEmoji} {moodName} 기분을 위한</Text>
            <Text style={styles.subtitle}>마음에 드는 영화를 선택해 주세요.</Text>
          </View>

          <View style={styles.progressContainer}>
              {/* 진행도 텍스트 수정 */}
              <Text style={styles.progressText}>{Math.min(currentIndex, displayTotal)}/{displayTotal}</Text>
          ...
          <View style={styles.deck}>
            {movies.slice(0, SWIPE_COUNT).map((movie, index) => { // 5개까지만 렌더링
